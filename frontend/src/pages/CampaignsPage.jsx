...
const PHASE_BAR = [
  { key: 'searching',  label: 'searching',  color: 'bg-slate-300' },
  { key: 'placed',     label: 'placed',     color: 'bg-blue-400' },
  { key: 'on_site',    label: 'on-site',    color: 'bg-workplace-teal-500' },
  { key: 'evaluating', label: 'evaluating', color: 'bg-emerald-500' },
  { key: 'completed',  label: 'completed',  color: 'bg-slate-200' },
];

function ProgrammeStats({ stats }) {
  if (!stats || stats.total === 0) {
    return <p className="text-xs text-slate-400">No students enrolled yet</p>;
  }
  return (
    <div>
      <div className="flex h-2 rounded-full overflow-hidden bg-slate-100 mb-2">
        {PHASE_BAR.map((p) => {
          ...
          return pct > 0 ? <div key={p.key} className={p.color} style={{ width: `${pct}%` }} title={`${stats[p.key]} ${p.label}`} /> : null;
        })}
      </div>
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
        <span>{stats.total} students</span>
        {PHASE_BAR.filter((p) => p.key !== 'completed').map((p) => (
          stats[p.key] > 0 && <span key={p.key}>{stats[p.key]} {p.label}</span>
        ))}
...
export default function CampaignsPage() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const c of filtered) {
      const label = c.academicYear?.label || 'No academic year';
    ...
    <div>
      ...
      >
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="ml-auto text-xs text-slate-400">{filtered.length} programme{filtered.length === 1 ? '' : 's'}</span>
        </div>
        {!isAdmin && (
        ...
        {isAdmin && <p className="text-xs text-slate-400 mt-2">Showing every coordinator's programmes (admin view)</p>}
      </PageHeader>
      ...
      {!loading && filtered.length === 0 && <p className="text-slate-400 text-sm">No programmes yet.</p>}
