...
// ── Completed summary ─────────────────────────────────────────────────────────
function CompletedSummary({ internship }) {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl shadow-sm p-6 border border-emerald-200">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-slate-800 text-lg">Internship completed</h3>
          <p className="text-xs text-slate-500">Finalized on {formatDate(internship.completed_at)}</p>
        </div>
      </div>
          ...
          {[teacherScore, supervisorScore].map((a, i) => a && (
            <div key={i} className="bg-white rounded-lg p-3">
              <div className="flex items-baseline justify-between">
                <span className="text-xs text-slate-400 capitalize">{a.assessor_role}</span>
