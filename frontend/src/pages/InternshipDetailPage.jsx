...
function ReviewCompleteForm({ internshipId, review, onDone }) {
  ...
  return (
    <form onSubmit={submit} className="mt-2 space-y-2">
      <textarea
        required placeholder={t('internshipDetail.reviews.reportPlaceholder')}
      ...
      <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-3 py-1.5">{t('internshipDetail.reviews.completeReview')}</button>
    </form>
  );
}
...
function ReviewsPanel({ internship, onSaved }) {
  ...
  async function cancel(reviewId) {
    if (!window.confirm(t('internshipDetail.reviews.cancelConfirm'))) return;
  ...
  return (
    <Section title={t('internshipDetail.reviews.title')}>
      <div className="space-y-2 mb-3">
        {(internship.reviews || []).length === 0 && <p className="text-sm text-slate-400">{t('internshipDetail.reviews.none')}</p>}
          ...
          <div key={r.id} className="border border-slate-100 rounded-lg p-2 text-sm">
            ...
            {r.status === 'scheduled' && (
              <p className="text-xs text-slate-400 mt-1">
                {t('internshipDetail.reviews.student')}: <span className={r.student_response === 'confirmed' ? 'text-emerald-600' : r.student_response === 'declined' ? 'text-red-600' : 'text-amber-600'}>{t(`internshipDetail.reviews.response.${r.student_response}`)}</span>
                {r.supervisor_id && <> · {t('internshipDetail.reviews.supervisor')}: <span className={r.supervisor_response === 'confirmed' ? 'text-emerald-600' : r.supervisor_response === 'declined' ? 'text-red-600' : 'text-amber-600'}>{t(`internshipDetail.reviews.response.${r.supervisor_response}`)}</span></>}
              </p>
            )}
            {r.status === 'completed' && (
              <>
                <p className="text-slate-600 mt-1">{r.report}</p>
                <p className="text-xs text-slate-400 mt-1">{t('internshipDetail.reviews.hoursLoggedAtReview', { hours: r.hours_logged_snapshot })}</p>
              </>
            )}
            {r.status === 'scheduled' && (
              completingId === r.id ? (
                ...
                <div className="flex gap-3 mt-1">
                  <button onClick={() => setCompletingId(r.id)} className="text-xs text-workplace-teal-700 hover:underline">{t('internshipDetail.reviews.complete')}</button>
                  <button onClick={() => cancel(r.id)} className="text-xs text-slate-400 hover:underline">{t('common.cancel')}</button>
                </div>
              )
            )}
          </div>
      ...
      <button onClick={() => setShowForm((s) => !s)} className="w-full py-2 border-2 border-dashed border-workplace-teal-200 text-workplace-teal-700 text-sm rounded-lg hover:bg-workplace-teal-50">
        {showForm ? t('common.cancel') : t('internshipDetail.reviews.scheduleReview')}
      </button>
        ...
        <form onSubmit={schedule} className="mt-3 space-y-2 border border-workplace-teal-100 rounded-lg p-3 bg-workplace-teal-50/30">
          ...
          {(internship.supervisors || []).length > 0 && (
            <select value={form.supervisor_id} onChange={(e) => setForm({ ...form, supervisor_id: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white">
              <option value="">{t('internshipDetail.reviews.supervisorAttending')}</option>
              {internship.supervisors.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          )}
          <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('internshipDetail.reviews.schedule')}</button>
        </form>
      )}
    </Section>
  );
...
export default function InternshipDetailPage() {
  ...
  async function advancePhase() {
    ...
    if (!window.confirm(t(`internshipDetail.phaseAdvanceConfirm.${currentPhase}`))) return;

    setError('');
    setPhaseMessage('');
    try {
      const updated = await api.post(`/api/internships/${id}/advance-phase`);
      setPhaseMessage(t('internshipDetail.movedTo', { phase: t(`phase.${updated.phase}`) }));
  ...
  async function reversePhase() {
    const reason = window.prompt(t('internshipDetail.reversePrompt'));
    if (reason === null) return; // cancelled
    if (!reason.trim()) { setError(t('internshipDetail.reverseReasonRequired')); return; }

    setError('');
    setPhaseMessage('');
    try {
      const updated = await api.post(`/api/internships/${id}/reverse-phase`, { reason: reason.trim() });
      setPhaseMessage(t('internshipDetail.movedBackTo', { phase: t(`phase.${updated.phase}`) }));
      setTimeout(() => setPhaseMessage(''), 4000);
      load();
    } catch (err) {
      setError(err.message);
  ...
  async function handleComplete(e) {
    e.preventDefault();
    if (!window.confirm(t('internshipDetail.completeConfirm'))) return;
    setError('');
    setCompleteMessage('');
    try {
      ...
      setCompleteMessage(t('internshipDetail.completeMarked'));
      load();
    } catch (err) {
      setError(err.message);
    }
  ...
  if (!internship) return <p className="text-slate-400 text-sm">{t('common.loading')}</p>;

  const supervisorLink = (token) => `${window.location.origin}/supervisor/${token}`;

  return (
    <div>
      <Link to="/campaigns" className="text-sm text-workplace-teal-700 hover:underline">{t('internshipDetail.backLink')}</Link>
      <div className="mt-2">
        <PageHeader
          title={`${internship.student?.first_name || ''} ${internship.student?.last_name || ''}`}
          subtitle={internship.phase === 'searching' ? t('internshipDetail.searchingSubtitle') : (internship.company_name || t('campaignDetail.noCompanyYet'))}
          actions={<PhaseProgress phase={internship.phase} />}
        >
          {(phaseMessage || PHASE_REVERSE[internship.phase] || PHASE_ADVANCE[internship.phase]) && (
            <div className="flex items-center justify-end gap-3">
              {phaseMessage && <span className="text-sm text-emerald-600">{phaseMessage}</span>}
              {PHASE_REVERSE[internship.phase] && (
                <button onClick={reversePhase} className="border border-slate-300 text-slate-600 hover:bg-slate-50 text-sm rounded-lg px-4 py-2">
                  ← {t(`internshipDetail.phaseReverse.${internship.phase}`)}
                </button>
              )}
              {PHASE_ADVANCE[internship.phase] && (
                <button onClick={advancePhase} className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">
                  {t(`internshipDetail.phaseAdvance.${internship.phase}`)}
                </button>
              )}
            </div>
          )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Section title={t('internshipDetail.company.title')}>
          <form onSubmit={saveCompany} className="space-y-2">
            <input placeholder={t('internshipDetail.company.namePlaceholder')} value={companyForm.company_name} onChange={(e) => setCompanyForm({ ...companyForm, company_name: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            <input placeholder={t('internshipDetail.company.addressPlaceholder')} value={companyForm.company_address} onChange={(e) => setCompanyForm({ ...companyForm, company_address: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            ...
            <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('common.save')}</button>
          </form>
        </Section>

        <Section title={t('internshipDetail.supervisors.title')}>
          <div className="space-y-2 mb-3">
            {(internship.supervisors || []).map((s) => (
              <div key={s.id} className="text-sm border border-slate-100 rounded-lg p-2">
                ...
                <p className="text-xs text-slate-500 break-all">{t('internshipDetail.supervisors.linkPrefix')}{supervisorLink(s.access_token)}</p>
              </div>
            ))}
          </div>
          <form onSubmit={addSupervisor} className="space-y-2">
            <input required placeholder={t('internshipDetail.supervisors.namePlaceholder')} value={supervisorForm.name} onChange={(e) => setSupervisorForm({ ...supervisorForm, name: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            <input placeholder={t('internshipDetail.supervisors.emailPlaceholder')} value={supervisorForm.email} onChange={(e) => setSupervisorForm({ ...supervisorForm, email: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            <input placeholder={t('internshipDetail.supervisors.jobTitlePlaceholder')} value={supervisorForm.job_title} onChange={(e) => setSupervisorForm({ ...supervisorForm, job_title: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('internshipDetail.supervisors.addSupervisor')}</button>
          </form>
        </Section>
        ...
        <Section title={t('internshipDetail.dailyLogs.title')}>
          ...
          <form onSubmit={addLog} className="space-y-2">
            <div className="flex gap-2">
              ...
              <input type="number" placeholder={t('internshipDetail.dailyLogs.hoursPlaceholder')} value={logForm.hours_logged} onChange={(e) => setLogForm({ ...logForm, hours_logged: e.target.value })} className="w-24 border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            </div>
            <textarea required placeholder={t('internshipDetail.dailyLogs.contentPlaceholder')} value={logForm.content} onChange={(e) => setLogForm({ ...logForm, content: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" rows={2} />
            <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('internshipDetail.dailyLogs.addLog')}</button>
          </form>
        </Section>

        <Section title={t('internshipDetail.assessment.title')}>
          <div className="space-y-2 mb-3">
          ...
          {(() => {
            ...
            return (
              <div className="text-sm border border-slate-100 rounded-lg p-2 mb-3 bg-slate-50">
                <p className="font-medium text-slate-700">{t('internshipDetail.assessment.studentReflection')} {studentReflection.submitted_at ? <span className="text-xs text-emerald-600 font-normal">{t('internshipDetail.assessment.submitted')}</span> : <span className="text-xs text-amber-600 font-normal">{t('internshipDetail.assessment.draft')}</span>}</p>
                {studentReflection.reflection && <p className="text-slate-600 mt-1">{studentReflection.reflection}</p>}
                {studentReflection.competency_notes && <p className="text-slate-500 mt-1 text-xs">{t('internshipDetail.assessment.competencies')}{studentReflection.competency_notes}</p>}
              </div>
            );
          })()}
          <form onSubmit={submitAssessment} className="space-y-2">
            <input type="number" placeholder={t('internshipDetail.assessment.scorePlaceholder')} value={assessmentForm.score} onChange={(e) => setAssessmentForm({ ...assessmentForm, score: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            <textarea placeholder={t('internshipDetail.assessment.feedbackPlaceholder')} value={assessmentForm.feedback} onChange={(e) => setAssessmentForm({ ...assessmentForm, feedback: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" rows={2} />
            <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('internshipDetail.assessment.submit')}</button>
          </form>
        </Section>

        <Section title={t('internshipDetail.checklist.title')}>
          <ChecklistLoader internshipId={id} onToggle={toggleCheck} />
        </Section>

        {internship.phase === 'evaluating' && (
          <Section title={t('internshipDetail.complete.title')}>
            <p className="text-sm text-slate-500 mb-3">{t('internshipDetail.complete.autoCalcNote')}</p>
            <form onSubmit={handleComplete} className="space-y-2">
              <textarea placeholder={t('internshipDetail.complete.notePlaceholder')} value={completeForm.completion_note} onChange={(e) => setCompleteForm({ ...completeForm, completion_note: e.target.value })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" rows={2} />
              {completeMessage && <p className="text-sm text-emerald-600">{completeMessage}</p>}
              <button type="submit" className="bg-workplace-teal-600 hover:bg-workplace-teal-700 text-white text-sm rounded-lg px-4 py-2">{t('internshipDetail.complete.markComplete')}</button>
            </form>
          </Section>
        )}
      ...
      {internship.phaseHistory && internship.phaseHistory.length > 0 && (
        <details className="mt-4 text-sm">
          <summary className="text-slate-400 hover:text-slate-600 cursor-pointer select-none">
            {t('internshipDetail.phaseHistory', { count: internship.phaseHistory.length })}
          </summary>
          <div className="mt-2 space-y-2 bg-white border border-slate-200 rounded-xl p-4">
            {internship.phaseHistory.map((h) => (
              <div key={h.id} className="text-slate-500">
                <span className="font-medium text-slate-700">{t(`phase.${h.from_phase}`, h.from_phase)} → {t(`phase.${h.to_phase}`, h.to_phase)}</span>
                {' — '}"{h.reason}"
...
function ChecklistLoader({ internshipId, onToggle }) {
  ...
  if (!items) return <p className="text-sm text-slate-400">{t('common.loading')}</p>;
